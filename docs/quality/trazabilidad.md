...
| **RF-07** Visualización de la ruta A* en mapa | `interfaces/api/web.py` + templates HTMX/Jinja2 + Leaflet | `vista_despacho` (`/consola/despacho`) + `red_vial` ([ADR-0022](../architecture/decisions/0022-rescate-frontend-htmx.md)) | `test_api_consola_despacho.py` + verificación visual | Ruta A* renderizada sobre tiles OSM con marcadores de incidente y unidad asignada | ✅ (consola web, bonus H5 — ADR-0022) |
...
| **RF-09** Panel de unidades en tiempo real | `interfaces/api/web.py` (vista `/consola/unidades`) | `vista_unidades` + `_cargar_unidades` ([ADR-0022](../architecture/decisions/0022-rescate-frontend-htmx.md)) | Verificación funcional durante FTR-02 | Estado actualizado refleja transiciones Disponible↔EnRuta↔EnEscena↔Taller con coordenadas | ✅ (consola web — ADR-0022) |
...
| **RN-10** Autenticación obligatoria + HTTPS | `interfaces/api` | Middleware FastAPI (pendiente) | _Auth + HTTPS fuera del scope v1; no se abordó en H4_ | Auth + HTTPS quedan fuera del scope v1 (diferido a v2): toda operación requeriría sesión autenticada y sin HTTPS rechazaría la conexión | 🟡 diferido v2 |
