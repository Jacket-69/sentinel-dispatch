...
| 4. Auditoría de productos | FTR por hito crítico (ver tabla siguiente) | Actas en `docs/quality/ftr/NNNN-*.md` |
...
| FTR | Producto auditado | Fecha | Hito | Estado |
|---|---|---|---|---|
| **FTR-0003** | Cierre técnico de H4 (log JSONL, exportador, simulación, spike CP-12, calibración CP-01c) + revisión de cobertura, paridad RT-02 y ADRs 0018-0020 | 2026-05-21 | Cierre H4 | Ejecutada — acta `docs/quality/ftr/0003-h4-cierre.md` |

Productos de mayor riesgo ya cerrados y validados en hitos previos, revisados en PR estándar y verificados por la suite y el CI dual:

- **A*** (`core-python/.../domain/routing/`): cerrado, cubierto por la suite Python.
- **Función de costo + política de re-despacho** (`core-python/.../domain/dispatch/`): cerrada y validada.
- **Núcleo Java + equivalencia dual** (RT-01..RT-04): cerrado — **186 tests JUnit** verdes y job CI `compare` en **12/12 bit-exacto** (RT-02).
...
- **FTR por hito crítico** (no calendarizado fijo): el acta FTR-0003 formalizó el cierre de H4; A*, función de costo y validación dual — los productos de mayor riesgo — quedaron cerrados y validados (186 tests Java, RT-02 12/12 bit-exacto) y se revisaron en PR estándar. Documentación declarativa se revisa en PR estándar.
