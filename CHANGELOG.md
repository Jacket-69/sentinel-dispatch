...
### Added — Consola web: vista de validación dual RT-02 (2026-06-12)
- **Nueva vista `GET /consola/validacion`** ("VALIDACIÓN DUAL"): muestra lado a lado los cálculos de ambos núcleos (core-python y core-java) sobre los 12 incidentes del dataset del SRS, evidenciando la paridad RT-02 como argumento visual de la defensa. Tabla por incidente con categoría MPDS, unidad asignada, ETA, costo total y ruta de cada núcleo, delta real (%) en los numéricos y veredicto por fila; banner-tablero con el agregado (N/N OK, deltas máximos, conteo WARN/FAIL).
- **Comparación server-side** (`interfaces/api/validacion.py`): espeja las tolerancias del validador canónico del CI (`tools/compare_outputs.py`) — match exacto en categoría/unidad/suboptimalidad/motivo, numéricos (ETA, costos) con tolerancia ±5 % mostrando el delta real, ruta con extremos exactos + largo ±10 %. La comparación corre en cada render (barata, <1 ms); inyectada en la vista vía `Depends`. Veredicto OK/WARN/FAIL por fila + resumen agregado.
- **Fixtures commiteadas** en `data/validacion/{python,java}/` (un `*.jsonl` por incidente, schema ADR-0017): outputs reales generados con la misma sintaxis del job `compare` del CI sobre el grafo de Coquimbo. Procedencia y comandos en `data/validacion/README.md`. Resultado actual: **12/12 OK · 0 WARN · 0 FAIL · delta máximo 0,000 %** (bit-exacto). El job CI `compare` valida lo mismo sobre outputs frescos en cada PR.
- **Nav**: `validacion` agregada a `_VISTAS_TODAS` (no al default `triaje,unidades`); etiqueta "VALIDACIÓN" en la barra; sigue habilitable por `SENTINEL_VISTAS`. Estética CRT coherente con las demás vistas (banner de flota, badges pill, reveal escalonado). `VERSION_ESTATICOS` bumpeado.
- 14 tests nuevos: 10 unit del comparador (paridad exacta, numérico dentro/fuera de tolerancia, extremos de ruta distintos, archivo faltante → MISSING, directorios inexistentes → vacío) + 4 de integración (GET 200, las 12 filas en paridad, presencia/ausencia en nav según `SENTINEL_VISTAS`). Suite Python **343** verde (`-m "not slow"` 339 + 4 slow), cobertura 92,7 % (gate 90). **Sin tocar dominio ni path operativo: RT-02 intacto.**

### Added — Deploy: consola pública en sentinel.threnody.me (ADR-0005 reactivado, 2026-06-09)
- **[ADR-0005](docs/architecture/decisions/0005-deploy-demo.md) `deferred` → `accepted`**: Cloudflare Tunnel desde el PC servidor, implementado con 3 desvíos del plan original documentados en el ADR (named tunnel + dominio propio en vez de `trycloudflare` efímero; **systemd user units** en vez de docker-compose; healthcheck como **systemd timer** en vez de cron). La app escucha solo en loopback (`127.0.0.1:8000`); el único camino público es el túnel (TLS + DDoS en el edge de Cloudflare).
- Zona DNS del dominio migrada a Cloudflare (Free) con los registros de correo verificados antes y después del cambio de nameservers. Verificación e2e: `/healthz`, `/consola/triaje` y `/consola/unidades` → 200 vía edge con certificado válido.
- `Makefile`: nuevo target **`dev`** (el README lo documentaba pero no existía): levanta la consola con `uvicorn --reload` en `:8000`.
- RN-10 (auth) sigue diferido a sabiendas: demo académica; si se necesita restringir, Cloudflare Access se antepone al hostname sin tocar la app (anotado en el ADR).

### Added — Consola web: puente triaje → despacho (balizas) + cache-busting (2026-06-09)
- **Puente triaje → despacho**: cada clasificación del triaje genera un **incidente de demo** en un punto aleatorio de la conurbación (snapeado a la red vial vía `nodo_mas_cercano` cuando el grafo está cargado; punto crudo del bbox urbano como fallback) y lo deja como **baliza pendiente** en un overlay en memoria (`app.state.incidentes_pendientes`, mismo patrón ADR-0022 que la flota viva). El fragmento de resultado enlaza "IR A DESPACHO".
- El mapa de despacho pinta las balizas como **anillos rojos pulsantes con tooltip** (`id · categoría`); clickear una fija coordenadas + categoría en el segmentado y deja el despacho a un botón. Despachar con unidad asignada **consume la baliza** (y el evento del log sale con su `incidente_id`); `POST /consola/despacho/reset` ahora también las descarta. Nuevo `GET /consola/despacho/incidentes`.
- **Cache-busting de estáticos**: `?v=<VERSION_ESTATICOS>` (global Jinja, bump manual en `web.py`) en todos los `<link>`/`<script>` de `/static` — sin esto, los navegadores que ya visitaron la consola servían CSS viejo cacheado tras cada cambio visual.
- 4 tests de integración nuevos (clasificar genera baliza; despacharla la consume y loguea su id; id inexistente cae a `I-CONSOLA`; reset descarta). Suite integración **48/48** verde. ⚠️ `test_cp12_50_unidades_p95_bajo_criterio_ajustado` (perf, margen 3 %) flakeó hoy en local (p95≈2058 ms) — flake conocido, independiente de estos cambios (no tocan dominio/application).

### Changed — Consola web: pulido visual de las 4 vistas y nav configurable (2026-06-09)
- **Nav configurable por env `SENTINEL_VISTAS`** (CSV de `triaje,despacho,unidades,log`). Default actual: `triaje,unidades` (modo presentación). Controla **solo la navegación**: las cuatro rutas siguen registradas y accesibles por URL directa, y la vista activa siempre aparece en la nav aunque no esté habilitada (deep links y tests intactos). Se evalúa en cada render (global Jinja invocable).
- **Vista de triaje**: layout en dos columnas (formulario | resultado sticky) bajo `consola--ancha`; los seis `<select>` se reemplazan por **controles segmentados** (radios estilizados, mismos `name`/`value` → el endpoint POST no cambia) con escalación de color clínica: opciones que gatillan Delta/Echo en rojo (`opcion--crit`), escaladas en ámbar (`opcion--warn`). El fragmento de resultado suma la **escala MPDS** (Alpha→Echo) con la categoría activa iluminada, animación de materialización por pasos y pulso de alarma para `crit`.
- **Panel de unidades**: **tablero de la flota** con contadores grandes por estado (disponible/en ruta/en escena/taller/total, calculados en plantilla), chips de tipo **ALS/BLS**, badges de estado tipo pill con fondo tenue, reveal escalonado de filas y barrido de hover con barra de selección.
- **Atmósfera CRT global** (`crt.css` + `base.html`): animación de **encendido del tubo** en cada carga, **viñeta** fija sobre el contenido, **aberración cromática** sutil en títulos, marcador `▶` en la vista activa de la nav, **reloj local vivo** en la status bar (JS mínimo inline) y soporte `prefers-reduced-motion`.
- **Vista de despacho**: el `<select>` de categoría → **control segmentado Alpha→Echo** con la misma escalación de color (Charlie `--warn`, Delta/Echo `--crit`); los estilos del segmentado se movieron de `triaje.css` a `crt.css` (lenguaje compartido). Mapa con **esquinas HUD**, cursor de mira, **ruta A\* con guiones en movimiento** (`stroke-dasharray` animado vía `className` del polyline) y **baliza pulsante** en el marcador de incidente. Panel de resultado con materialización al renderizar y pulso de alarma en la variante crítica. `despacho.js`: lee la categoría del radio marcado (`categoriaSeleccionada()`), sin cambios de flujo.
- **Vista de log**: contadores por severidad (OK/WARN/CRIT/INFO) en la barra superior, tipo de evento como **chip de nivel**, reveal escalonado por línea (delay acotado a 900 ms para logs largos) y **prompt `$ █` parpadeante** al final como tail vivo del terminal.
- Tests: `test_get_vista_unidades_ok` actualizado a la nav por default + nuevo `test_nav_completa_con_sentinel_vistas`. Integración 44/44 verde. **Sin tocar dominio ni path operativo: RT-02 intacto.**

### Added — Consola web: despacho con mapa + consola viva (ADR-0022, 2026-06-03)
- **Despacho con mapa (RF-07)**: `GET /consola/despacho` (mapa Leaflet sin tiles, fondo negro) + `POST /consola/despacho/despachar` (form `lat`/`lon`/`categoria_mpds`). El backend carga el grafo OSM una vez al arranque (FastAPI lifespan, `app.state`, inyectado vía dependencia `obtener_grafo` para tests con grafo fake), corre `despachar()` y devuelve `serializar_resultado_despacho` (ADR-0017) + un bloque `geo` con coords `[lat, lon]` para Leaflet (incidente, base de la unidad, ruta A*, distancia de snap RN-09). El front-end dibuja la ruta en verde fósforo, marcadores de incidente/unidad y panel con unidad/ETA/motivo. Snap fuera de la IV Región → 422; grafo no cargado → 503.
- **Wireframe de la red vial**: `GET /consola/despacho/red-vial` sirve las calles principales (`OsmnxGrafoVial.calles_principales`, filtro `motorway`/`trunk`/`primary`/`secondary`/`tertiary` + `_link`, dedup no dirigido, precomputado en lifespan): ~5.4k segmentos / ~290 KB, dibujados como líneas tenues en un pane de fondo bajo la ruta. Evita renderizar las 42 508 aristas completas (prohibitivo).
- **Consola viva (estado en memoria)**: al despachar, la unidad elegida se marca `EnRuta` en un overlay `app.state.estados_unidades` que el **panel de unidades refleja** y que **excluye a esa unidad del siguiente despacho** (simula flota viva). El despacho además registra un evento `despacho_creado` en el **log JSONL** (timestamp con sufijo `Z` y `despacho_id` `SD-<YYYYMMDD>-<seq>` por ADR-0018), visible en la vista de log. `POST /consola/despacho/reset` libera la flota. Es una conveniencia de la capa de interfaces: no persiste, no toca el dominio ni RT-02 (ver ADR-0022 §Relación con SRS y RT-02).
- 13 tests de integración del despacho (grafo fake vía `dependency_overrides`): página, ruta+geo, 422 fuera de región, 422 enum inválido, 503 sin grafo, marca EnRuta, escribe al log, panel refleja EnRuta, reset, red-vial, **saturación end-to-end, despachos consecutivos que consumen la flota, y reset que reincorpora**. 8 tests del adapter `calles_principales`. Fixture autouse en `tests/integration/conftest.py` aísla el estado en memoria entre tests. Suite Python **324** verde.
- Revisión adversarial multi-agente del diff: 28 hallazgos brutos → 18 confirmados; corregidos los de valor (timestamp `Z`, log de colisión observable, capas del wireframe, reencuadre en reset, despacho idempotente por incidente, manejo de 422 de validación, cobertura de saturación/multi-despacho).
